...
This analysis uses 3 covariates as drivers of the population's instrinic growth rate:
...
We begin by getting the daily flow data from the US Geological Service [National Water Information System](http://waterdata.usgs.gov/nwis). We will use the direct link to the gage data from the Skagit River near Mount Vernon, WA (#12200500), begining with the first year of fish data.
...
The column names in the data file are `agency_cd`, `site_no`, `datetime`, `01_00060_00003`, and `01_00060_00003_cd`. The daily flow measurements are in column `01_00060_00003`, so we will only keep `datetime` and `01_00060_00003`, and rename them to `date` and `flow`, respectively.

```{r trim_dat_flow}
dat_flow <- dat_flow[c("datetime","01_00060_00003")]
...
flow_aut <- subset(dat_flow, (month>=10 & month<=12)
                   & year >= yr_frst & year <= yr_last-age_min+n_fore+flow_lag-1)
## spring flows in year t+1
flow_spr <- subset(dat_flow, (month>=1 & month<=3)
                   & year >= yr_frst+1 & year <= yr_last-age_min+n_fore+flow_lag)
...
dat_flow <- aggregate(flow ~ year, data=rbind(flow_aut,flow_spr), max)
## for plotting purpose later
colnames(dat_flow)[2] <- "Flow"
```
...
dat_PDO <- read.table("http://research.jisao.washington.edu/pdo/PDO.latest",
                      ...
                      skip=hdr_pdo + yr_frst - 1900 + 1, nrows=n_yrs)
colnames(dat_PDO) <- unlist(strsplit(tolower(PDO_raw[hdr_pdo]), split="\\s+"))
dat_PDO
```

We want the average PDO index from April through October during the first six months the juvenile steelhead are in the ocean (i.e., during their second year of life). Therefore, we need PDO values from `yr_frst + PDO_lag` = `r yr_frst+PDO_lag` through `yr_last - age_min + n_fore + PDO_lag` = `r yr_last - age_min + n_fore + PDO_lag`.

Also notice that values in the year column in the PDO data file contains an ** superscript from 2002 onward to indicate the switch to version 2 of the sea surface temperature (SST) data. We will fix those as well.

```{r adj_PDO}
## fix 'year' col
dat_PDO[,"year"] <- seq(yr_frst,yr_last)
## mean PDO from Apr-Oct indexed by brood year
dat_PDO <- apply(dat_PDO[dat_PDO[,"year"] %in% seq(yr_frst+PDO_lag,yr_last-age_min+n_fore+PDO_lag),
                         tolower(month.abb)[4:10]],1,mean)
dat_PDO <- data.frame(year=seq(yr_frst,yr_last-age_min+n_fore), PDO=dat_PDO)
```

### Hatchery releases

The numbers of hatchery fish released each year is listed in a file on the project site. They have already been lagged 2 years (i.e., brood year + 2) to account for the potential competitive interactions during their juvenile life stage. 
...
```{r combine_covars}
## covariate(s)
dat_cvrs <- merge(merge(dat_flow,dat_PDO),dat_hrel)
## drop year col
dat_cvrs <- dat_cvrs[,-1]
...
model {
	...
	ln_Rkr_a[1] <- mu_Rkr_a + c_Flow*dat_cvrs[1,1] + c_PDO*dat_cvrs[1,2] + c_Hrel*dat_cvrs[1,3];
	...
	for(t in 2:(n_yrs-age_min+n_fore)) {
		# predicted recruits in BY t
		ln_Rkr_a[t] <- mu_Rkr_a + c_Flow*dat_cvrs[t,1] + c_PDO*dat_cvrs[t,2] + c_Hrel*dat_cvrs[t,3];
...
par_jags <- c("alpha","E_Rkr_a","mu_Rkr_a","Rkr_b",
              "c_Flow","c_PDO","c_Hrel",
...
init_vals <- function() {
#	list(mu_Rkr_a=1, c_Flow=runif(1,-0.1,0.1), c_PDO=runif(1,-0.1,0.1), c_Hrel=runif(1,-0.1,0.1),
	list(mu_Rkr_a=1, c_Flow=rnorm(1,0,0.1), c_PDO=rnorm(1,0,0.1), c_Hrel=rnorm(1,0,0.1),
...
```{r tbl_summary_stats}
print(mod_fit$BUGSoutput$summary[c("E_Rkr_a","alpha","Rkr_b",
                                   "c_Flow","c_PDO","c_Hrel",
                                   "var_Qr","var_Rs"),
...
Here is the relationship between spawner and subsequent recruits (a), assuming mean values for all covariates. Gray lines show 100 possible relationships based on random draws from the posterior distributions for the productivity (b) and density-dependence parameters (c). Note that for plotting purposes only in (b) and (c), the density in the largest bin for each parameter contains counts for all values greater or equal to that. Vertical arrows under the x-axes in (b) and (c) indicate the 2.5^th^, 50^th^, and 97.5^th^ percentiles.

```{r plot_SR, fig.width=8, fig.height=5, fig.pos="placeHere"}
...
rDat <- exp(rDat)
ln_a <- exp(matrix(mod_fit$BUGSoutput$sims.array[,,"E_Rkr_a"],ncol=1))
bb <- matrix(mod_fit$BUGSoutput$sims.array[,,"Rkr_b"], ncol=1)

## plot of spawner-recruit relationship
dd <- 3000
...
for(i in 1:MC) { lines(ln_a[idx[i]]*seq(xM)*exp(-bb[idx[i]]*seq(xM)), col="darkgray") }
...
## posterior for 1/b
par(mai=c(0.8,0.4,0.3,0.1))
R_b_est <- 1/mod_fit$BUGSoutput$sims.list$Rkr_b
R_b_CI <- quantile(R_b_est,c(0.025,0.5,0.975))
R_b_est[R_b_est>2e4] <- 2e4
brks <- seq(Re2prec(min(R_b_est),"floor",2000),2e4,500)
hist(R_b_est, freq=FALSE, breaks=brks, col=clr, border="blue3",
	 xlab="", xaxt="n", yaxt="n",
	 main="", ylab="", cex.lab=1.2)
axis(1, at=seq(Re2prec(min(R_b_est),"floor",2000),2e4,4000))
aHt <- (par()$usr[4]-par()$usr[3])/12
arrows(R_b_CI,par()$usr[3],R_b_CI,par()$usr[3]-aHt,
       code=1,length=0.05,xpd=NA,col="blue3",lwd=1.5)
mtext(expression(paste("Carrying capacity (",1/italic(b),")")), 1, line=3, cex=1)
text(x=par()$usr[1]+par()$pin[2]/par()$pin[1]*offSet*diff(par()$usr[1:2]),
...
Here are time series plots of the standardized covariates (a-c) and histograms of their effects on productivity (d-f).

```{r plot_cov_effects, fig.width=8, fig.height=8.5, fig.pos="placeHere", warnings=FALSE, messages=FALSE}
offSet <- 0.07
c_est <- mod_fit$BUGSoutput$sims.matrix[,grep("c_",names(mod_fit$BUGSoutput$sims.list),value=TRUE)]
par(mfrow=c(ncol(c_est),2), mai=c(0.4,0.2,0.1,0.1), omi=c(0.2,0.4,0,0))
...
for(i in 1:ncol(c_est)) {
	...
	if(i==ncol(c_est)) { mtext(side=1,"Brood year", line=3) }
	## plot covar effect
	hist(c_est[,grep(colnames(dat_cvrs)[i],colnames(c_est))],
	     freq=FALSE,breaks=brks,col=clr,border="blue3",
	     xlab="", yaxt="n", main="", ylab="", cex.axis=1.2)
	c_CI <- quantile(c_est[,grep(colnames(dat_cvrs)[i],colnames(c_est))],c(0.025,0.5,0.975))
	...
	text(x=par()$usr[1]+par()$pin[2]/par()$pin[1]*offSet*diff(par()$usr[1:2]), cex=1.2,
	     y=par()$usr[4]-offSet*diff(par()$usr[3:4]),paste0("(",letters[i+ncol(c_est)],")"))
	if(i==ncol(c_est)) { mtext(side=1,"Effect size", line=3) }
}
```
...
```{r plot_run_size, fig.width=6, fig.height=4, fig.pos="placeHere"}
...
pDat <- apply(mod_fit$BUGSoutput$sims.list$Sp,2,quantile,CI.vec)
